...
cat > /ect/apache2/sites-available/$PROJECTID"_"$STAGE.conf << EOF
<VirtualHost *:80>
  ...
  ErrorLog \${APACHE_LOG_DIR}/"$PROJECTID"_"$STAGE"_error.log
  LogLevel warn
  CustomLog \${APACHE_LOG_DIR}/"$PROJECTID"_"$STAGE"__access.log combined
</VirtualHost>" >
EOF
